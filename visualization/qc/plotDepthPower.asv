function info = plotDepthPower(rec, varargin)
...
%   .t_start_s, .win_s, .fs, .nCyc
    ...
    switch lower(o.Normalize)
        case 'freq'
            ...
            cbLabel = 'LFP power (dB re. median depth)';
        otherwise
            cbLabel = 'LFP power (dB)';
    end
    ...
    if ~isempty(o.clim)
        ...
        cl = prctile_(powerDB(:), o.climPct);
        if diff(cl) <= 0, cl = [min(powerDB(:)) max(powerDB(:))]; end
    end
    
    % ---------------------------------------------------------------- plot
    figure('Color','w','Position',[100 100 900 760]);
    imagesc(f, zsorted, powerDB, cl);
    ...
    title(sprintf('probe00a LFP power by depth (Morlet)  |  T=%g s, %g s window  |  %s', ...
          t0, win_s, o.Normalize));
    cb = colorbar; cb.Label.String = cbLabel;
    box on;
    
    fprintf(['LFP depth-spectrum: %d/%d channels (z<=%g), %d freqs %.3g-%.3g Hz, ' ...
             'cyc %.3g-%.3g, %g s window @ %g Hz\n'], ...
        nKept, nChan, o.z_max, o.nFreq, fMin, fMax, cyc(1), cyc(end), win_s, fs);
    
    %% output / save

    info = struct('f', f, 'depth', zsorted, 'powerDB', powerDB, ...
                  't_start_s', t0, 'win_s', win_s, 'fs', fs, 'nCyc', cyc);

    if o.SaveToggle
        figType = 'lfp';
        figName = sprintf('lfpSpectrogram_t%g_w%g.png', t0, win_s);
